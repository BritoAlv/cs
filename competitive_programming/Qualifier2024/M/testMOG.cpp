...
void solve()
{
    ...
    if (ans)
    {
        ...
        vector<vector<pair<pair<int, int>, int>>> D(n);
        vector<int> dur(n, 0);
        for (int seg_i = 0; seg_i < seg_active.size(); seg_i++)
        {
            for (int i = 0; i < m; i++)
            {
                ...
                {
                    if (D[z.second].size() > 0 && D[z.second].back().second == i &&
                        D[z.second].back().first.second == z.first.first)
                    {
                        D[z.second].back().first.second += (z.first.second - z.first.first);
                    }
                    else
                    {
                        D[z.second].push_back({{z.first.first, z.first.second}, i});
                    }
        ...
        forn
        {
            ...
            for (auto &z : D[i])
            {
                cout << z.second + 1 << wp << z.first.first << wp << z.first.second << wp;
            }
            cout << endl;
        }
    }
