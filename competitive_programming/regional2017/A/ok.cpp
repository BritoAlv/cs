...
int main()
{
    ...
    forn
    {
        ...
        {
            int c1 = j;
            int c2 = ((int)P.size() + j - 1) % (int)P.size();
            if (P[c1].second < P[c2].second)
            {
                //printf("Segment from (%d,%d) to (%d, %d) \n", P[c1].first + shift, P[c1].second, P[c2].first + shift,P[c2].second);
                segments[i].insert(Segment(P[c1].first + shift, P[c1].second, P[c2].first + shift, P[c2].second));
            }
            else
            {
                //printf("Segment from (%d,%d) to (%d, %d) \n", P[c2].first + shift, P[c2].second, P[c1].first + shift,P[c1].second);
                segments[i].insert(Segment(P[c2].first + shift, P[c2].second, P[c1].first + shift, P[c1].second));
            }
        }
        for (int j = 0; j < P.size(); j++)
        {
